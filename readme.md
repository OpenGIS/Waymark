...
- Added the ability to show/hide all Types initially when using the Shortcode by specifying "_" as the Type key. For example, to initially hide all Marker Types, except "photo" and "alert", use `[Waymark map_id="1234" hide_marker="_" show_marker="photo,alert"]`. Thanks to [Association Franc-Comtoise du Chemin de Compostelle](https://www.af-ccc.fr) for requesting this.
...
- **Global Callback** - If the function `waymark_loaded_callback` is defined globally, it will be called when Waymark has loaded. This allows you to extend Waymark functionality similar to the Shortcode Callback, however it will apply to _all_ Waymark Maps.
...
- [Map First](https://github.com/opengis/map-first) - A minimal WordPress theme with an _obsession_ for Maps. It's open-source too and contains lots of comments about customisations.
